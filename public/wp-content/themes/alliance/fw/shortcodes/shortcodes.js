...
function themerex_init_shortcodes(container) {
	...
	if (container.find('.sc_countdown:not(.inited)').length > 0) {
		container.find('.sc_countdown:not(.inited)')
			.each(function () {
				...
				if( style == 3){
					var destDate = new Date(endDateParts[0], endDateParts[1]-1, endDateParts[2], endTimeParts[0], endTimeParts[1], endTimeParts[2]);
					var diff = Math.round(destDate.getTime() / 1000 - curDate.getTime() / 1000);
			
					jQuery(this).FlipClock(destDate, {
						countdown: true,
	...
	// Review form
	if (container.find('.reviews_block').length > 0) {
		jQuery('.reviews_block .reviews_stars.reviews_style_heart:not(.reviews_editable)').each(function(){
			var data = jQuery(this).data('mark') + '%';
			jQuery(this).css('width', data);
		});
	}
	...
	if (container.find('.sc_islands_item:not(.inited)').length > 0) {
		themerex_init_islands(container);
		jQuery(window).scroll(function () { themerex_init_islands(container); });
	}
...
// Islands init
function themerex_init_islands(container) {
...
function themerex_countdown(dt) {
	...
	for (var i=2; i<dt.length; i++) {
		...
		counter.find('.sc_countdown_item').eq(i-2).find('.sc_countdown_digits span').addClass('hide');
		for (var ch=v.length-1; ch>=0; ch--) {
			counter.find('.sc_countdown_item').eq(i-2).find('.sc_countdown_digits span').eq(ch+(i==2 && v.length<2 ? 1 : 0)).removeClass('hide').text(v.substr(ch, 1));
		}
	}
}
