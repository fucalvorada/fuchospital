...
(function() {
    ...
    function() { //begin Curve
        ...
        var t = this,
            ...
            s = {
                scaleShowGridLines: !0,
                scaleGridLineColor: "rgba(231,232,225,0)",
                scaleGridLineWidth: 1,
                bezierCurve: !0,
    ...
    function() { //begin Line
        ...
        var t = this,
            ...
            s = {
                scaleShowGridLines: !0,
                scaleGridLineColor: "rgba(231,232,225,1)",
                scaleGridLineWidth: 1,
                bezierCurve: false,
                bezierCurveTension: .4,
                pointDot: !0,
...
jQuery(document).ready(function(){
	...
	//graph
	if (jQuery('.tw-chart-graph:not(.inited)').length > 0) {
		chart_graph_init(jQuery('body').eq(0));
		jQuery(window).scroll(function () { chart_graph_init(jQuery('body').eq(0)); });
        jQuery('.tw-chart-graph:not(.inited)').parents('.sc_tabs').find('.tab_names').click(function () { chart_graph_init(jQuery('body').eq(0)); });
    }
...
//graph
function chart_graph_init(container) {

	if (arguments.length==0) var container = jQuery('body');
		var scrollPosition = jQuery(window).scrollTop() + jQuery(window).height();

		container.find('.tw-chart-graph:not(.inited)').each(function () {
			"use strict";
        
			var item = jQuery(this);
           
			var scrollSkills = item.offset().top + 300;
			if (scrollPosition > scrollSkills) {
				...
				var $datasets = [];
                var $graphWidth = jQuery(this).width();
				...
				jQuery('>ul.data>li',$curr).each(function(){ 
					...
					if($currData.attr('data-fill-text')&&$currData.attr('data-fill-color')){$listDesc+='<li><i class="fa-caret-right fa" style="background:'+$currData.attr('data-fill-color')+';"></i>'+$currData.attr('data-fill-text')+'</li>';}
					$datasets.push(
						{
							// Line And Radar
							...
							pointHighlightStroke: 'rgba('+hex2rgb($currData.attr('data-fill-color')).red+','+hex2rgb($currData.attr('data-fill-color')).green+','+hex2rgb($currData.attr('data-fill-color')).blue+',1)',
							// Bar
							highlightFill    : 'rgba('+hex2rgb($currData.attr('data-fill-color')).red+','+hex2rgb($currData.attr('data-fill-color')).green+','+hex2rgb($currData.attr('data-fill-color')).blue+',1)',
							highlightStroke  : 'rgba('+hex2rgb($currData.attr('data-fill-color')).red+','+hex2rgb($currData.attr('data-fill-color')).green+','+hex2rgb($currData.attr('data-fill-color')).blue+',1)',
							// All
							label            : $currData.attr('data-fill-text'),
							fillColor        : 'rgba('+hex2rgb($currData.attr('data-fill-color')).red+','+hex2rgb($currData.attr('data-fill-color')).green+','+hex2rgb($currData.attr('data-fill-color')).blue+',1)',
							strokeColor      : $currData.attr('data-fill-color'),
							data : $currData.attr('data-datas')?$currData.attr('data-datas').split(','):[" "]
						}
					);
				});
				if($graphWidth > 0) if($listDesc){$curr.addClass('with-list-desc').before('<div class="graph list-desc" style="width: ' + $graphWidth + 'px; margin: auto;"><ul>'+$listDesc+'</ul></div>');}
				else if($listDesc){$curr.addClass('with-list-desc').before('<div class="graph list-desc"><ul>'+$listDesc+'</ul></div>');}
                
				$canvas.attr('width', $curr.width()+'px');
				$canvas.attr('height',$height===''?$curr.height()+'px':$height);
				...
				if($curr.hasClass('tw-animate'))
				{
					...
					setTimeout(function(){
						...
						$height = ($height==='')?($curr.height()+'px'):$height;
						$canvas.attr('width', $curr.width()+'px');
						$canvas.attr('height', $height); //graph height
