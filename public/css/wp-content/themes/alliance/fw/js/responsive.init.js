...
function themerex_ready_actions() {
	...
	// Menu
    //----------------------------------------------
    jQuery('.menu_main_wrap .menu_main_nav > li > a').click(function(){
	...
	//subsub menu
  	jQuery('.menu_main_wrap .menu_main_nav > li > ul > li > a').click(function(){
    	var item = jQuery(this).parent();
    	jQuery(item).addClass('open');
    });
...
function themerex_resize_elements(){
	var windowWidth = jQuery(window).width(); 	

	if(windowWidth <= 480){
		//register form
		var agree_field = jQuery('.popup_wrap.popup_registration .form_left .popup_form_field.agree_field').clone();
		var submit_field = jQuery('.popup_wrap.popup_registration .form_left .popup_form_field.submit_field').clone();
		jQuery('.popup_wrap.popup_registration .form_right').append(agree_field).append(submit_field);
		jQuery('.popup_wrap.popup_registration .form_left .popup_form_field.agree_field, .popup_wrap.popup_registration .form_left .popup_form_field.submit_field').remove();
	}


	//slider
	jQuery('.sc_slider').each(function(){
		var x = jQuery(this).attr('data-old-width');
		var y = jQuery(this).attr('data-old-height');
		var z = x / y;

		var newX = jQuery(this).width();
		var newY = newX / z;
		jQuery(this).height(newY);
		
		if(jQuery(this).width() > 600){
			jQuery(this).removeClass('responsive_2, responsive_2, responsive_3');
		}
		if(jQuery(this).width() <= 600){
			jQuery(this).addClass('responsive_1').removeClass('responsive_2, responsive_3');
		}
		if(jQuery(this).width() <= 480){
			jQuery(this).addClass('responsive_2').removeClass('responsive_3');
		}
		if(jQuery(this).width() <= 320){
			jQuery(this).addClass('responsive_3');
		}
	
	});
	
	//countdown
	jQuery('.sc_countdown.sc_countdown_style_3').each(function(){
		if(jQuery(this).width() > 520){
			jQuery(this).removeClass('responsive_1');
		}
		if(jQuery(this).width() <= 520){
			jQuery(this).addClass('responsive_1').removeClass('responsive_2');
		}
		if(jQuery(this).width() <= 470){
			jQuery(this).addClass('responsive_2').removeClass('responsive_3');
		}
		if(jQuery(this).width() <= 455){
			jQuery(this).addClass('responsive_3').removeClass('responsive_4');
		}
		if(jQuery(this).width() <= 380){
			jQuery(this).addClass('responsive_4').removeClass('responsive_5');
		}
		if(jQuery(this).width() <= 250){
			jQuery(this).addClass('responsive_5');
		}
	...
	//template masonry
	jQuery('.sc_blogger.template_masonry,  .sc_blogger.template_classic').each(function(){
		var sizes = '';
		sizes = jQuery(this).data('size');

		if(sizes != ''){
			var array = sizes.split('x');
			var N = parseInt(array[0]) / parseInt(array[1]);
			var x = jQuery(this).find('.isotope_item').width();
			var y = x / N;
			jQuery(this).find('.isotope_item').height(y);
		}
	});


	//template portfolio
	jQuery('.sc_blogger.template_portfolio').each(function(){
		var sizes = '';
		sizes = jQuery(this).data('size');
		...
		if(sizes != ''){
			...
			var x = jQuery(this).find('.isotope_item').width();
			var y = x / N;
			jQuery(this).find('.isotope_item .post_item').height(y);
		}
	});

	
	//template news
	jQuery('.sc_blogger.sc_blogger_horizontal.layout_news').each(function(){
		if(jQuery(this).width() < 280) jQuery(this).addClass('responsive');
		else jQuery(this).removeClass('responsive');
	});
	

}

function widthInPercent(x,y){
	var N = x * 100 / y
 	return N;
}